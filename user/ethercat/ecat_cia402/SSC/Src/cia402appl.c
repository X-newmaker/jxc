...
extern mcl_user_value_t motor_torque;
extern void motor_torque_loop_set(int32_t target_torque,int32_t max_motor_speed);
extern void motor_torque_loop_init(void);
#endif
/*--------------------------------------------------------------------------------------
------
...
void CiA402_DummyMotionControl(TCiA402Axis *pCiA402Axis)
{
    ...
    !pCiA402Axis->bBrakeApplied)
    {
            ...
            || pCiA402Axis->Objects.objPositionActualValue < pCiA402Axis->Objects.objTargetPosition))
        {
            ...
            case CYCLIC_SYNC_TORQUE_MODE:
            	motor_torque_loop_set(pCiA402Axis->Objects.objTargetTorque,pCiA402Axis->Objects.objMaxProfileVelocity);
            	break;
            default:
                break;
            }
    ...
    pCiA402Axis->Objects.objTorqueActualValue = motor_get_actual_torque();
...
void CiA402_Application(TCiA402Axis *pCiA402Axis)
{
    ...
    if(pCiA402Axis->u16PendingOptionCode != 0 &&
		...
		pCiA402Axis->Objects.objModesOfOperationDisplay == CYCLIC_SYNC_TORQUE_MODE))
    {
...
//                LocalAxes[PDOAssignEntryCnt].Objects.objSupportedDriveModes = 0x80;
            	LocalAxes[PDOAssignEntryCnt].Objects.objSupportedDriveModes = (1 << (HOMING_MODE - 1)) |
																			  (1 << (CYCLIC_SYNC_POSITION_MODE - 1)) |
																			  (1 << (CYCLIC_SYNC_VELOCITY_MODE - 1)) |
																			  (1 << (CYCLIC_SYNC_TORQUE_MODE - 1)) ;
        ...
        case 0://copy csp TxPDO entries
            {
				...
				#else//dynamic pdo
                for(u16 i = 0; i < LocalAxes[AxisIndex].Objects.sTxPDOMap0.u16SubIndex0; i++)
                {
                	u8 dlen = (LocalAxes[AxisIndex].Objects.sTxPDOMap0.aEntries[i] >> 3) & 0x7;

                	switch(LocalAxes[AxisIndex].Objects.sTxPDOMap0.aEntries[i])
                	{
                	case 0x60410010://ObjStatusWord
                		memcpy(pTmpData, &LocalAxes[AxisIndex].Objects.objStatusWord, dlen);
                		break;
                	case 0x60640020://ObjPositionActualValue
                		memcpy(pTmpData, &LocalAxes[AxisIndex].Objects.objPositionActualValue, dlen);
                		break;
                	case 0x60610008://ObjModesOfOperationDisplay
                		memcpy(pTmpData, &LocalAxes[AxisIndex].Objects.objModesOfOperationDisplay, dlen);
                		break;
                	case 0x606C0020://objVelocityActualValue
						memcpy(pTmpData, &LocalAxes[AxisIndex].Objects.objVelocityActualValue, dlen);
						break;
                	case 0x60770010://objTorqueActualValue
                		memcpy(pTmpData, &LocalAxes[AxisIndex].Objects.objTorqueActualValue, dlen);
					break;
                	default://error
...
void APPL_OutputMapping(UINT16* pData)
{
    ...
    for (j = 0; j < sRxPDOassign.u16SubIndex0; j++)
    {
        ...
        case 0:    //csp RxPDO    entries
			...
			#else //dynamic pdo
            for(u16 i = 0; i < LocalAxes[AxisIndex].Objects.sRxPDOMap0.u16SubIndex0; i++) {

            	u8 dlen = (LocalAxes[AxisIndex].Objects.sRxPDOMap0.aEntries[i] >> 3) & 0x7;

            	switch(LocalAxes[AxisIndex].Objects.sRxPDOMap0.aEntries[i])
				{
				case 0x60400010://objControlWord
					memcpy((u8 *)(&LocalAxes[AxisIndex].Objects.objControlWord), pTmpData, dlen);
					break;
				case 0x60FF0020://objTargetVelocity
					memcpy((u8 *)(&LocalAxes[AxisIndex].Objects.objTargetVelocity), pTmpData, dlen);
					break;
				case 0x60600008://objModesOfOperation
					memcpy((u8 *)(&LocalAxes[AxisIndex].Objects.objModesOfOperation), pTmpData, dlen);
					break;
				case 0x607A0020://objTargetPosition
					memcpy((u8 *)(&LocalAxes[AxisIndex].Objects.objTargetPosition), pTmpData, dlen);
					break;
				case 0x60710010://objTargetTorque
					memcpy((u8 *)(&LocalAxes[AxisIndex].Objects.objTargetTorque), pTmpData, dlen);
					break;
				case 0x60720010://objMaxTorque
					memcpy((u8 *)(&LocalAxes[AxisIndex].Objects.objMaxTorque), pTmpData, dlen);
					break;
				case 0x607F0020://
					memcpy((u8 *)(&LocalAxes[AxisIndex].Objects.objMaxProfileVelocity), pTmpData, dlen);
					break;
				case 0x60800020://
					memcpy((u8 *)(&LocalAxes[AxisIndex].Objects.objMaxMotorSpeedValue), pTmpData, dlen);
					break;
				default://error
...
UINT8 ObjWrite0x6060( UINT16 Index, UINT8 Subindex, UINT32 Size, UINT16 MBXMEM * pData, UINT8 bCompleteAccess)
{
   ...
   if ( Subindex == 0 )
   {
      if (*pData == CYCLIC_SYNC_POSITION_MODE) {
          ...
          motor_torque_loop_init();
          LocalAxes[0].Objects.objModesOfOperation = CYCLIC_SYNC_TORQUE_MODE;
      }
      else
