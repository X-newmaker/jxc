...
#define SPI_WAIT_DELAY_US       1
#define HAL_QSPI_WAIT_PER_CYCLE      SPI_WAIT_DELAY_US
#define HAL_QSPI_MIN_FREQ_HZ         100000
#define QSPI_TRANSFER_DATA_LEN_1M   0x100000
#define QSPI_CPU_DMA_MIN_SPEED_MS   (0x800000 >> 10)
#define SPI_RET_ETIMEDOUT		110

#define hal_log_err printk
#define aic_udelay(x) gtc_dly_time(GTC_US, x);
...
#define HAL_QSPI_WAIT_DELAY_US 50
...
#define PHACTL_TX_PHASE_EN_MASK         BIT(15)
#define PHACTL_RX_PHASE_EN_MASK         BIT(12)
#define PHACTL_TX_DATA_MASK             BIT(14)
#define PHACTL_TX_DATA_SET_VAL(v)       ((v) & PHACTL_TX_DATA_MASK)
#define PHACTL_TX_CLK_MASK              GENMASK(2, 0)
#define PHACTL_TX_CLK_SET_VAL(v)        ((v) & PHACTL_TX_CLK_MASK)
#define PHACTL_RX_PHASE_OFS             (4)
#define PHACTL_RX_PHASE_SET_MASK        (0XFUL << PHACTL_RX_PHASE_OFS)
#define PHACTL_RX_PHASE_SET_VAL(v)      (((v) << PHACTL_RX_PHASE_OFS) & PHACTL_RX_PHASE_SET_MASK)
...
#define BMCLK_BMRX_PHAST_MSK	BIT(28)
...
#define HAL_QSPI_INPUT_MAX_FREQ_HZ_V2           200000000
#define HAL_QSPI_INPUT_MIN_FREQ_HZ_V2           63000000
...
static inline void qspi_hw_set_tx_delay_mode(u32 base, u32 tx_dlymode)
{
    u32 val = readl(QSPI_REG_PHACTL(base));
    val &= ~(PHACTL_TX_PHASE_EN_MASK);
    val |= PHACTL_TX_DATA_SET_VAL(tx_dlymode);
    val |= PHACTL_TX_CLK_SET_VAL(tx_dlymode);
...
{
    u32 val = readl(QSPI_REG_PHACTL(base));
    val &= ~(PHACTL_RX_PHASE_EN_MASK);
    val &= ~(PHACTL_RX_PHASE_SET_MASK);
    val |= PHACTL_RX_PHASE_SET_VAL(mode);
    writel(val, QSPI_REG_PHACTL(base));
}

static inline u32 qspi_hw_get_tx_delay_mode(u32 base)
{
    u32 val = readl(QSPI_REG_PHACTL(base));
    return val & PHACTL_TX_DATA_MASK;
}

static inline u32 qspi_hw_get_rx_delay_mode(u32 base)
{
    u32 val = readl(QSPI_REG_PHACTL(base));
    return (val & PHACTL_RX_PHASE_SET_MASK) >> PHACTL_RX_PHASE_OFS;
}

static inline void qspi_hw_set_rx_delay_normal(u32 base, u32 mode)
{
    u32 val = readl(QSPI_REG_TCFG(base));
...
static inline void qspi_hw_set_delay_mode_normal(u32 base)
{
    u32 val = readl(QSPI_REG_PHACTL(base));
    val = 0;
    val |= (PHACTL_RX_PHASE_EN_MASK);
...
static inline void qspi_hw_set_tx_delay_normal(u32 base, u32 en)
{
    u32 val = readl(QSPI_REG_TCFG(base));
...
static inline void qspi_hw_dma_word_enable(u32 base, bool en)
{
    u32 val = readl(QSPI_REG_FCTL(base));
    if (en)
