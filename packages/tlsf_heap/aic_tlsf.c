...
void aic_tlsf_heap_test(void)
{
    ...
    pr_debug("\n\nStart malloc:\n\n");
    for (i=0; i<10; i++){
        tmp[i] = aic_tlsf_malloc(0, len[i]);
        pr_debug("%s: heap region %d malloc %d len.\n ",  __func__, 0, len[i]);
        aic_tlsf_mem_info(0, &total_m, &used_m, &max_used, &free_m, &max_free);
        pr_debug("%s: heap region %d total=%d, used=%d, max_used=%d, free=%d, max_free=%d.\n",
                __func__, 0, total_m, used_m, max_used, free_m, max_free);
    }

    pr_debug("\n\nStart free:\n\n");
    for (i=0; i<10; i++){
        aic_tlsf_free(0, tmp[i]);
