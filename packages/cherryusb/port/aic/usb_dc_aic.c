...
#else
#define USB_RAM_SIZE 2048  /* define with maximum value*/
#endif
...
#define AIC_UDC_PHYCTL_REG	(USB_BASE + 0x8000)
/* Endpoint state */
struct aic_ep_state {
    ...
    uint8_t inep_timeout_flag;
};

extern uint32_t usbd_clk;

/* Driver state */
/*USB_NOCACHE_RAM_SECTION*/USB_NOCACHE_RAM_SECTION struct aic_udc {
...
#ifdef CONFIG_USB_AIC_DMA_ENABLE
USB_NOCACHE_RAM_SECTION static uint8_t g_aic_udc_ibuf[USB_RAM_SIZE] __ALIGNED(CACHE_LINE_SIZE);
USB_NOCACHE_RAM_SECTION static uint8_t g_aic_udc_obuf[USB_RAM_SIZE] __ALIGNED(CACHE_LINE_SIZE);
...
static void aic_tx_fifo_empty_procecss(uint8_t ep_idx)
{
    ...
    while (((AIC_UDC_REG->nptxfifosts & INEPTXSTS_IN_EP_TXFIFO_STS) >= len32b)
            && ((AIC_UDC_REG->nptxfifosts & INEPTXSTS_IN_EP_TXQUEUE_STS) != 0)
            && AIC_UDC_REG->inepcfg[ep_idx] & DEPCTL_EPENA
...
#ifndef CONFIG_USB_AIC_DMA_ENABLE
    AIC_UDC_REG->outepintmsk |= CTRL_OUT_EP_SETUP_PHASE_DONE | CTRL_OUT_EP_SETUP_RCVD | OUTTK_EPDIS;
...
int usb_dc_init(void)
{
    ...
    for (uint8_t i = 0U; i < UDC_TXFIFO_NUM; i++) {
    ...
    AIC_UDC_REG->usbintmsk = INT_RESET | INT_ENUMDONE |
                             INT_OUT_EP | INT_IN_EP | INT_GINNAKEFF |
...
int usbd_inep_reset(void)
{
	...
	for (i = 0U; i < USB_NUM_BIDIR_ENDPOINTS; i++) {
	    ...
	    if (j == DIS_EP_TIMOUT)
	        USB_LOG_ERR("%s: ep%d timeout OUTEPCFG.EPDisable %#lx\n", __func__, fail-1, AIC_UDC_REG->inepint[i]);
	    ...
	    fail = 0;
	    AIC_UDC_REG->usbdevfunc |= USBDEVFUNC_CGNPINNAK;
	    while(AIC_UDC_REG->usbintsts & INT_GINNAKEFF) {
	    	fail++;
	    	USB_LOG_WRN("%d %#lx\n", fail, AIC_UDC_REG->usbintsts);
	    }

}

int usbd_npinep_resend(const uint8_t ep)
{
    int fail = 0;
    unsigned int pending_map = 0;
    int cnt = 0;
    ...
    if (g_aic_udc.in_ep[ep].inep_timeout_flag == 1)
    	g_aic_udc.in_ep[ep].inep_timeout_flag = 0;
    else
    	return 0;

    uint32_t pktcnt = (uint16_t)((g_aic_udc.in_ep[ep].xfer_len + g_aic_udc.in_ep[ep].ep_mps - 1U) / g_aic_udc.in_ep[ep].ep_mps);
    uint32_t lost_packets = (AIC_UDC_REG->ineptsfsiz[ep] & (DXEPTSIZ_PKT_CNT_MASK)) >> DXEPTSIZ_PKT_CNT_SHIFT;
    remain_size = g_aic_udc.in_ep[ep].xfer_len - (pktcnt - lost_packets) * g_aic_udc.in_ep[ep].ep_mps;

    if (lost_packets == 0U)
    	goto __end;

    if (g_aic_udc.in_ep[ep].xfer_len != 0) {
        g_aic_udc.in_ep[ep].xfer_len = remain_size;
        g_aic_udc.in_ep[ep].xfer_buf -= remain_size;
        g_aic_udc.in_ep[ep].actual_xfer_len = 0;
        if ((AIC_UDC_REG->ineptsfsiz[ep] | DXEPTSIZ_XFER_SIZE_MASK) != remain_size && remain_size != 0)
        	AIC_UDC_REG->ineptsfsiz[ep] |= (DXEPTSIZ_XFER_SIZE_MASK & remain_size);
    }
__end:
	AIC_UDC_REG->inepcfg[ep] |= (DEPCTL_CNAK | DEPCTL_EPENA);

    fail = 0;
    while((AIC_UDC_REG->inepcfg[ep] & DEPCTL_EPENA) == 0U) {
    	fail++;
    	aicos_udelay(1);
    	if (fail > DIS_EP_TIMOUT) {
    		USB_LOG_WRN("ep%d timeout\n", ep);
    		break;
    	}
    }
    ...
    if (AIC_UDC_REG->outepcfg[ep_idx] & DEPCTL_EPENA) {
        ...
        if (i == 100000) {
            ...
            USB_LOG_ERR("ep%d AIC_UDC_REG->outepcfg[ep_idx] = 0x%x.\n",
                        ep_idx, AIC_UDC_REG->outepcfg[ep_idx]);
            return -2;
        }
    }
...
extern u32 ep0_ctlwr_rd_flg;	//ep0 control write transfer read flag
u32 usb_suspend_flg = 0x0;
u64 usb_suspend_time0 = 0x0;
u64 usb_suspend_time1 = 0x0;

ATTRIBUTE_ISR void USBD_IRQHandler(void)
{
    ...
    uint32_t temp;
    uint32_t read_count;
    uint8_t *setup = (uint8_t *)&g_aic_udc.setup;
        ...
        while (ep_intr != 0U) {
            if ((ep_intr & 0x1U) != 0U) {
                ...
                if ((ep_idx == 0)) {
                    ...
                    if ((epint & OUTTK_EPDIS) == OUTTK_EPDIS) {
                    	if (ep0_ctrl_stage == AIC_EP0_DATA_IN) {
                    		usbd_inep_reset();
                    		aic_ep0_start_read_setup((uint8_t *)&g_aic_udc.setup);
                    	}
                    }
                    ...
                    #endif
                    if ((epint & OUTTK_EPDIS) == OUTTK_EPDIS) {
                    	AIC_UDC_REG->outepcfg[ep_idx] |= DEPCTL_CNAK | DEPCTL_EPENA | DEPCTL_USBACTEP;
                    }
					#if 1
                    ep->actual_xfer_len = ep->xfer_len - ((AIC_UDC_REG->outeptsfsiz[ep_idx]) & DXEPTSIZ_XFER_SIZE_MASK);
                    ep->xfer_len = 0;
    ...
    if (gint_status & INT_IN_EP) {
        ...
        while (ep_intr != 0U) {
            if ((ep_intr & 0x1U) != 0U) {
                ...
                if (epint & NON_ISO_IN_EP_TIMEOUT) {
                	usbd_inep_reset();
            	    if (g_aic_udc.in_ep[ep_idx].xfer_len == 0U)
            	    	AIC_UDC_REG->inepcfg[ep_idx] |= (DEPCTL_CNAK | DEPCTL_EPENA);
            	    else
            	    	g_aic_udc.in_ep[ep_idx].inep_timeout_flag = 1;
                }

				#ifndef CONFIG_USB_AIC_DMA_ENABLE
				if ((epint & INTKN_TXFEMP) == INTKN_TXFEMP) {
					usbd_npinep_resend(ep_idx);
    ...
    if (gint_status & INT_RESET) {
        ...
        if (usb_suspend_flg == 0x1) {
        	usb_suspend_flg = 0x0;						//clear usb_suspend_flg
        	usb_suspend_time1 = gtc_get_time(GTC_MS);
        	usb_suspned_td = usb_suspend_time1 - usb_suspend_time0;	//get delta time

        	if (usb_suspned_td > 1000) {		//over 1sec, reset usb controller
        		usb_dc_deinit();
        		usb_dc_init();
        	} else {
        		usb_dc_rst();
    ...
    if (gint_status & INT_SUSPEND) {
        ...
        if (usb_suspend_flg == 0x0) {
        	usb_suspend_time0 = gtc_get_time(GTC_MS);	//get gtc time
        }
        usb_suspend_flg = 0x1;
